...
echo "🚀 Starting compilation of PageRank action using cluster-identical environment..."

docker run --rm --entrypoint="" \
    -v "$PROJECT_ROOT/pagerank/ow-pr":/tmp/input_actions \
...
if [ $? -eq 0 ]; then
    ...
    mkdir -p "$PROJECT_ROOT/pagerank/ow-pr/bin"
    cp "$PROJECT_ROOT/pagerank/ow-pr/exec_cluster" "$PROJECT_ROOT/pagerank/ow-pr/bin/exec"
    chmod +x "$PROJECT_ROOT/pagerank/ow-pr/bin/exec"
    echo "📦 Binary is ready at: pagerank/ow-pr/bin/exec"
else
