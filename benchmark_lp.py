...
def benchmark_standalone(graph_file, num_nodes, max_iter):
    ...
    binary_path = "labelpropagation/lpst/target/release/label-propagation"
    
    if not os.path.exists(binary_path):
        print(f"Error: Binary not found at {binary_path}", file=sys.stderr)
        print("Run: cd labelpropagation/lpst && cargo build --release", file=sys.stderr)
        return None
...
def run_validation(graph_file, num_nodes, bucket, key, endpoint):
    ...
    val_result = subprocess.run([
        "python3", "labelpropagation/validate_results.py",
