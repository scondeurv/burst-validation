...
def run_prst(num_nodes, max_iter, local_graph):
    prst_bin = "./pagerank/prst/target/release/label-propagation"
    print("\n--- Running PRST (Single-Threaded Rust) ---")
    if not os.path.exists(prst_bin):
        print("Error: prst binary not found. Please compile it first with 'cargo build --release'")
        return None

    cmd = [prst_bin, local_graph, str(num_nodes), str(max_iter)]
    start = time.time()
    result = subprocess.run(cmd, capture_output=True, text=True)
    end = time.time()
    
    if result.returncode != 0:
        print(f"Error running prst: {result.stderr}")
        return None
    
    data = json.loads(result.stdout)
    data['total_real_time_ms'] = (end - start) * 1000
    print(f"PRST Finished in {data['total_real_time_ms']:.2f} ms")
    return data
...
def run_ow_lp(num_nodes, max_iter, partitions, memory, granularity):
    ...
    # Use parameters from run_pagerank.sh
    cmd = [
        sys.executable, "pagerank/pagerank.py",
        "--ow-host", ow_host,
        "--ow-port", str(ow_port),
        "--pr-endpoint", lp_endpoint,
...
def main():
    ...
    max_iter = args.iter
    memory = args.memory
    local_graph = f"pagerank_{num_nodes}.txt"

    prst_res = run_prst(num_nodes, max_iter, local_graph)
    run_ow_lp(num_nodes, max_iter, partitions, memory, granularity)

    if not os.path.exists("pagerank-burst.json"):
        print("Benchmarking failed. pagerank-burst.json not found.")
        return

    with open("pagerank-burst.json", "r") as f:
    ...
    if min_start == float('inf') or max_end == 0:
        print("Warning: Could not find valid worker timestamps in pagerank-burst.json")
        burst_time_ms = 0
    else:
    ...
    if prst_res:
        ...
        print("========================================")
        print(f"PRST Time:  {prst_res['total_real_time_ms']:.2f} ms")
        print(f"Burst Time: {burst_time_ms:.2f} ms")
        if burst_time_ms > 0:
            speedup = prst_res['total_real_time_ms'] / burst_time_ms
            format_char = "x"
        else:
            speedup = 0
            format_char = " (Error)"
